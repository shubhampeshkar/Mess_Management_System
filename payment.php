...
<form action="paymentsuccess.php" method="post">
            <div class="mb-3">
                ...
                <select class="form-select" id="card" name="card" required>
                    ...
                    <option value="John Doe">Debit Card</option>
                    <option value="Jane Smith">Credit Card</option>
                </select>
            </div>
            <div class="mb-3">
    ...
    $sql = "SELECT amount FROM planandpayments ORDER BY id DESC LIMIT 1";
    $result = $conn->query($sql);

    if ($result->num_rows > 0) {
        // Output data of each row
        while($row = $result->fetch_assoc()) {
            echo "<h6>Amount</h6>" . $row["amount"];
        }
    } else {
        echo "0 results";
    }
